...
public class MKTdashboardServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        MKTDashboardDAO dashboardDAO = new MKTDashboardDAO(); // Khởi tạo DAO
        int postCountpost = dashboardDAO.getCountPost(); // Lấy số lượng bài đăng từ DB
        int postCountproduct = dashboardDAO.getCountProduct();
        int postCountcustomer = dashboardDAO.getCountCustomer();
        int postCountfeedback = dashboardDAO.getCountFeedback();

        request.setAttribute("postCountpost", postCountpost); // Đặt biến vào request
